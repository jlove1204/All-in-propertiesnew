...
const SECRET=LIVE_ENABLED?(process.env.STRIPE_WEBHOOK_SECRET||''):(process.env.STRIPE_TEST_WEBHOOK_SECRET||process.env.STRIPE_WEBHOOK_SECRET||'');

function key(){if(LIVE_ENABLED)return LIVE_KEY.startsWith('sk_')?LIVE_KEY:'';if(TEST_KEY.startsWith('sk_test_'))return TEST_KEY;if(LIVE_KEY.startsWith('sk_test_'))return LIVE_KEY;return''}
...
async function ensurePortalToken(c,rid){
  const {data,error}=await c.from('reservations').select('guest_portal_token_hash').eq('id',rid).maybeSingle();if(error)throw error;if(data?.guest_portal_token_hash)return;
  const t=crypto.randomBytes(32).toString('hex'),now=new Date().toISOString();const {error:u}=await c.from('reservations').update({guest_portal_token_hash:hashToken(t),guest_portal_token_created_at:now}).eq('id',rid);if(u)throw u;
}
async function confirm(c,session){
  ...
  await ensurePortalToken(c,rid);
}
async function cancel(c,session){
  const rid=session.metadata?.reservation_id;if(!rid)return;
  const {error}=await c.from('reservations').update({status:'cancelled',cancelled_at:new Date().toISOString(),cancellation_reason:'Checkout expired or payment failed.'}).eq('id',rid).eq('status','pending');if(error)throw error;
}
async function handler(req,res){
  if(req.method!=='POST')return json(res,405,{error:'Method not allowed'});
  if(!U||!K||!key()||!SECRET)return json(res,503,{error:'Webhook not configured'});
  try{
    ...
    const st=new Stripe(key()),event=st.webhooks.constructEvent(raw,sig,SECRET),c=createClient(U,K,{auth:{persistSession:false,autoRefreshToken:false}});
